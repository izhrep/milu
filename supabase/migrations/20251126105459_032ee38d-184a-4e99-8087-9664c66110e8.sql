-- Исправляем функцию create_diagnostic_task_for_participant
-- Проблема: она пытается получить deadline_date напрямую из diagnostic_stages, 
-- но это поле находится в parent_stages
...
BEGIN
  ...
  IF self_assignment_id IS NOT NULL AND NOT EXISTS (
    ...
    INSERT INTO public.tasks (
      ...
      'self',
      'Выбрать оценивающих',
      'Выберите коллег для проведения оценки 360. Срок: ' || COALESCE(parent_deadline_date::text, 'не указан'),
      'pending',
  ...
  IF manager_user_id IS NOT NULL AND manager_user_id != NEW.user_id THEN
    ...
    IF manager_assignment_id IS NOT NULL AND NOT EXISTS (
      ...
      INSERT INTO public.tasks (
        ...
        'Необходимо пройти оценку 360 для ' || participant_full_name || '. Срок: ' || COALESCE(parent_deadline_date::text, 'не указан'),
        'pending',
        parent_deadline_date,
