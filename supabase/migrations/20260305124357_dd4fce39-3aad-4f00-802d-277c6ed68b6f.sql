...
-- =============================================================================
-- Wave 1: Diagnostic Config Templates — Core DDL
-- =============================================================================
...
-- CHECK: at least one score source
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'chk_score_source' AND conrelid = 'public.hard_skill_results'::regclass
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'chk_score_source' AND conrelid = 'public.soft_skill_results'::regclass
  ) THEN
...
CREATE POLICY "Read approved templates"
  ...
  USING (status = 'approved' OR public.has_permission(auth.uid(), 'diagnostics.manage'));
...
-- 2a. Approval validation trigger
...
DROP TRIGGER IF EXISTS validate_template_before_approval ON public.diagnostic_config_templates;
...
-- 2b. Freeze trigger on stage activation
CREATE OR REPLACE FUNCTION public.freeze_template_config_on_activation()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE
  tpl RECORD; hard_labels jsonb; soft_labels jsonb;
BEGIN
  IF OLD.frozen_config IS NOT NULL AND NEW.frozen_config IS DISTINCT FROM OLD.frozen_config THEN
    RAISE EXCEPTION 'frozen_config is immutable once set';
  END IF;
  IF NEW.status = 'active' AND (OLD.status IS DISTINCT FROM 'active')
     AND NEW.config_template_id IS NOT NULL AND NEW.frozen_config IS NULL THEN
    SELECT * INTO tpl FROM diagnostic_config_templates WHERE id = NEW.config_template_id;
    IF NOT FOUND THEN RAISE EXCEPTION 'Template % not found', NEW.config_template_id; END IF;
    IF tpl.status != 'approved' THEN RAISE EXCEPTION 'Template must be approved before stage activation'; END IF;
    SELECT COALESCE(jsonb_agg(jsonb_build_object('level_value', l.level_value, 'label_text', l.label_text) ORDER BY l.level_value), '[]'::jsonb)
    INTO hard_labels FROM template_scale_labels l
    WHERE l.template_id = tpl.id AND l.skill_type = 'hard' AND l.level_value BETWEEN tpl.hard_scale_min AND tpl.hard_scale_max;
    SELECT COALESCE(jsonb_agg(jsonb_build_object('level_value', l.level_value, 'label_text', l.label_text) ORDER BY l.level_value), '[]'::jsonb)
    INTO soft_labels FROM template_scale_labels l
    WHERE l.template_id = tpl.id AND l.skill_type = 'soft' AND l.level_value BETWEEN tpl.soft_scale_min AND tpl.soft_scale_max;
    NEW.frozen_config := jsonb_build_object(
      ...
      'hard_scale_reversed', tpl.hard_scale_reversed, 'soft_scale_reversed', tpl.soft_scale_reversed,
...
DROP TRIGGER IF EXISTS set_template_updated_at ON public.diagnostic_config_templates;
