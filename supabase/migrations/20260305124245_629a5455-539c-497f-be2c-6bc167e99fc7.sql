...
-- =============================================================================
-- Sync RPC functions: get_hard_skill_results_safe / get_soft_skill_results_safe
-- Adds raw_numeric_value to RETURNS TABLE signature
-- =============================================================================

-- Drop and recreate get_hard_skill_results_safe
DROP FUNCTION IF EXISTS public.get_hard_skill_results_safe(uuid, uuid);

CREATE OR REPLACE FUNCTION public.get_hard_skill_results_safe(
...
-- Drop and recreate get_soft_skill_results_safe
DROP FUNCTION IF EXISTS public.get_soft_skill_results_safe(uuid, uuid);

CREATE OR REPLACE FUNCTION public.get_soft_skill_results_safe(
  p_evaluated_user_id uuid,
  p_stage_id uuid DEFAULT NULL::uuid
)
RETURNS TABLE(
