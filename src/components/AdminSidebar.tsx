    ...
    <Sidebar className={state === 'collapsed' ? 'w-16' : 'w-64'}>
      <SidebarContent>
        <div className="p-4 border-b">
          <h2 className={`font-semibold ${state === 'collapsed' ? 'text-caption-sm text-center' : 'text-body-lg'}`}>
            {state === 'collapsed' ? 'Справ.' : 'Справочники'}
          </h2>
          {state !== 'collapsed' && (
            <button
              onClick={() => navigate('/')}
              className="mt-3 w-full flex items-center gap-2 px-3 py-2 text-body-md text-muted-foreground hover:text-primary hover:bg-accent rounded-lg transition-colors"
            >
