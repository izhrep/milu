...
import { Target, CheckCircle, ArrowRight } from "@/components/icons";
...
export const UserCareerTrackView: React.FC<UserCareerTrackViewProps> = ({ trackId }) => {
  ...
  if (!trackData) {
    return <div className="text-center py-8 text-muted-foreground">Трек не найден</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-card bg-gradient-subtle">
        <CardHeader>
          <div className="flex items-start justify-between">
            <div>
              <CardTitle className="text-heading-3">{trackData.name}</CardTitle>
              {trackData.description && (
                <p className="text-muted-foreground mt-2">{trackData.description}</p>
              )}
            </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            {trackData.target_positions && (
              <div>
                <p className="text-body-md text-muted-foreground">Целевая должность</p>
                <p className="font-medium">{trackData.target_positions.name}</p>
              </div>
            )}
            {trackData.duration_months && (
              <div>
                <p className="text-body-md text-muted-foreground">Продолжительность</p>
      ...
      <div className="space-y-4">
        <h3 className="text-body-lg font-semibold text-foreground">Этапы карьерного трека</h3>
                ...
                <Card key={step.id} className="border-0 shadow-card">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <CardTitle className="text-body-lg">{step.grades?.name}</CardTitle>
                          {step.grades?.description && (
                            <p className="text-body-md text-muted-foreground mt-1">{step.grades.description}</p>
                          )}
                        </div>
                      </div>
                  ...
                  <CardContent className="space-y-4">
                    {step.grades?.key_tasks && (
                      <div>
                        <p className="text-body-md font-medium text-foreground mb-2">Ключевые задачи:</p>
                        <p className="text-body-md text-muted-foreground">{step.grades.key_tasks}</p>
                      </div>
                    )}

                    {stepSkills.length > 0 && (
                      <div>
                        <p className="text-body-md font-medium text-foreground mb-2">Навыки:</p>
                    ...
                    {stepQualities.length > 0 && (
                      <div>
                        <p className="text-body-md font-medium text-foreground mb-2">Качества:</p>
                        <div className="flex flex-wrap gap-2">
                  ...
                  {index < stepsData.length - 1 && (
                    <div className="flex justify-center pb-4">
                      <ArrowRight className="h-6 w-6 text-muted-foreground/70" />
                    </div>
                  )}
                </Card>
              );
