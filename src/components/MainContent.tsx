...
const MainContent = () => {
  ...
  return (
    <>
      <main className="min-w-60 overflow-hidden flex-1 shrink basis-6 bg-muted pb-6 max-md:max-w-full">
      <div className="flex min-h-[70px] w-full items-center justify-center text-heading-2 text-foreground font-bold whitespace-nowrap text-center tracking-[-0.45px] leading-none py-[15px] max-md:max-w-full">
        <h1 className="text-foreground self-stretch my-auto">Milu</h1>
      </div>
      
      <ProfileHeader userData={userData} />
      
      <div className="w-full mt-9 px-3 max-md:max-w-full">
        <nav className="flex w-full items-center gap-5 text-heading-3 text-foreground font-medium leading-none px-3 py-2.5 max-md:max-w-full">
            ...
            <div className="flex min-w-60 w-full items-center gap-5 flex-wrap flex-1 shrink basis-[0%] max-md:max-w-full">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`text-foreground self-stretch my-auto transition-opacity hover:opacity-100 ${
        ...
        <div className="w-full mt-3 max-md:max-w-full">
          <div className="w-full max-md:max-w-full">
            <div className="flex w-full items-center gap-2.5 text-body-lg text-foreground font-medium tracking-[-0.18px] leading-none justify-center pt-5 pb-2.5 px-3 max-md:max-w-full">
              <h2 className="text-foreground self-stretch flex-1 shrink basis-[0%] my-auto max-md:max-w-full">
                Общая информация
              </h2>
            </div>
            
            <section className="w-full max-md:max-w-full">
              <div className="shadow-card flex w-full gap-6 flex-wrap bg-white p-6 rounded-[20px] max-md:max-w-full max-md:px-5">
                
                <div className="min-w-60 flex-1 shrink basis-[0%] max-md:max-w-full">
                  <h3 className="text-foreground text-body-md font-medium leading-none">
                    Руководители
                  </h3>
                    ...
                    {supervisorsLoading ? (
                      ...
                      supervisors.map((supervisor, index) => (
                         <div key={supervisor.id} className={`flex items-center gap-[19px] ${index > 0 ? 'mt-5' : ''}`}>
                           <div className="aspect-[1] w-10 h-10 bg-warning rounded-full flex items-center justify-center text-white text-body-md font-semibold">
                             {getInitials(getFullName(supervisor))}
                           </div>
                           <div className="self-stretch flex flex-col w-[214px] my-auto pr-9 rounded-[0px_0px_0px_0px]">
                             <div className="text-foreground text-body-md font-medium">
                               {getFullName(supervisor)}
                             </div>
                             <div className="text-muted-foreground text-caption-sm font-normal mt-1.5">
                               {supervisor.role}
                             </div>
                           </div>
                         </div>
                      ...
                      <div className="flex items-center justify-center py-4">
                        <p className="text-muted-foreground text-caption-sm">Руководители не найдены</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <Collapsible open={expandedSections.personal} onOpenChange={(open) => setExpandedSections(prev => ({ ...prev, personal: open }))}>
                <div className="w-full mt-2.5 pb-3 px-3 max-md:max-w-full">
                  <CollapsibleTrigger className="flex w-full items-center gap-[40px_100px] justify-between flex-wrap max-md:max-w-full hover:opacity-80 transition-opacity">
                    <h3 className="text-foreground text-body-lg font-medium leading-none tracking-[-0.18px] self-stretch my-auto">
                      Личная информация
                    </h3>
                    <div className="self-stretch flex items-center text-caption-sm text-warning font-normal whitespace-nowrap leading-none my-auto">
                      <span className="text-warning self-stretch w-[68px] my-auto">
                        {expandedSections.personal ? 'Свернуть' : 'Развернуть'}
                      </span>
                      <ChevronDown className={`w-6 h-6 text-warning transition-transform ${expandedSections.personal ? 'rotate-180' : ''}`} />
                    </div>
                  </CollapsibleTrigger>
                </div>
                
                <CollapsibleContent className="w-full">
                  <div className="shadow-card flex w-full gap-6 flex-wrap bg-white p-6 mx-3 rounded-[20px] max-md:max-w-full max-md:px-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                      <div>
                        <h4 className="text-foreground text-body-md font-medium mb-3">Контактная информация</h4>
                        <div className="space-y-2 text-caption-sm">
                          <p className="text-muted-foreground">Email: {userData?.email || 'Не указан'}</p>
                          <p className="text-muted-foreground">Телефон: {userData?.user_profiles?.phone || 'Не указан'}</p>
                          <p className="text-muted-foreground">Дата рождения: {userData?.user_profiles?.birth_date ? new Date(userData.user_profiles.birth_date).toLocaleDateString() : 'Не указана'}</p>
                        </div>
                      </div>
                      <div>
                        <h4 className="text-foreground text-body-md font-medium mb-3">Рабочая информация</h4>
                        <div className="space-y-2 text-caption-sm">
                          <p className="text-muted-foreground">Табельный номер: {userData?.employee_number || 'Не указан'}</p>
                          <p className="text-muted-foreground">Дата принятия: {userData?.start_date ? new Date(userData.start_date).toLocaleDateString() : 'Не указана'}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="w-full mt-2.5 pb-3 px-3 max-md:max-w-full">
                  <CollapsibleTrigger className="flex w-full items-center gap-[40px_100px] justify-between flex-wrap max-md:max-w-full hover:opacity-80 transition-opacity">
                    ...
                    <div className="self-stretch flex items-center text-caption-sm text-warning font-normal whitespace-nowrap leading-none my-auto">
                      ...
                      <ChevronDown className={`w-6 h-6 text-warning transition-transform ${expandedSections.skills ? 'rotate-180' : ''}`} />
                    </div>
                  </CollapsibleTrigger>
                </div>
                  ...
                  <div className="space-y-6 mx-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="bg-white p-6 rounded-[20px] shadow-card">
                        <h4 className="text-foreground text-body-base font-semibold mb-4">Результаты оценки 360</h4>
                        {survey360Loading ? (
                          <div className="flex items-center justify-center py-4">
                            <p className="text-muted-foreground text-caption-sm">Загрузка...</p>
                          </div>
                          ...
                          <div className="space-y-3">
                            <div className="grid grid-cols-3 gap-2 text-center">
                              <div className="p-3 bg-primary/10 rounded-lg">
                                ...
                                <div className="text-muted-foreground text-caption-sm">Самооценка</div>
                              </div>
                              <div className="p-3 bg-success/10 rounded-lg">
                                <div className="text-success text-body-lg font-bold">
                                  {survey360AggregatedResults.overall_summary.average_supervisor_score.toFixed(1)}
                                </div>
                                <div className="text-muted-foreground text-caption-sm">Руководитель</div>
                              </div>
                              <div className="p-3 bg-chart-3/10 rounded-lg">
                                <div className="text-chart-3 text-body-lg font-bold">
                                  {survey360AggregatedResults.overall_summary.average_colleague_score.toFixed(1)}
                                </div>
                                <div className="text-muted-foreground text-caption-sm">Коллеги</div>
                              </div>
                            </div>
                            <div className="p-3 bg-warning/10 rounded-lg text-center">
                              <div className="text-warning text-heading-4 font-bold">
                                {survey360AggregatedResults.overall_summary.overall_average.toFixed(1)}
                              </div>
                              <div className="text-muted-foreground text-caption-sm">Общий балл</div>
                            </div>
                            <div className="text-muted-foreground text-caption-sm text-center">
                              Soft Skills оценено: {survey360AggregatedResults.overall_summary.total_qualities}
                            </div>
                          </div>
                        ) : (
                          <div className="text-center py-4">
                            <p className="text-muted-foreground text-caption-sm">Нет результатов оценки 360</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="bg-white p-6 rounded-[20px] shadow-card">
                        <h4 className="text-foreground text-body-base font-semibold mb-4">Результаты оценки Hard Skills</h4>
                        {skillAssessmentLoading ? (
                          ...
                          <div className="space-y-3">
                            ...
                            {skillAggregatedResults.overall_summary.priority_skills.length > 0 && (
                              <div className="mt-3">
                                <div className="text-foreground text-caption-sm font-medium mb-2">Приоритетные для развития:</div>
                                <div className="space-y-1">
                                  {skillAggregatedResults.overall_summary.priority_skills.slice(0, 3).map((skill) => (
                                    <div key={skill.skill_id} className="flex justify-between items-center text-caption-sm">
                                      <span className="text-muted-foreground">{skill.skill_name}</span>
                                      <span className="text-destructive font-medium">-{skill.gap_analysis.toFixed(1)}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                          ...
                          <div className="text-center py-4">
                            <p className="text-muted-foreground text-caption-sm">Нет результатов оценки Hard Skills</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="shadow-card flex w-full gap-6 flex-wrap bg-white p-6 rounded-[20px] max-md:max-w-full max-md:px-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                        {/* Навыки */}
                        <div>
                          <h4 className="text-foreground text-body-base font-semibold mb-4">Навыки</h4>
                          {assessmentResultsLoading ? (
                              ...
                              {skillResults.slice(0, 5).map((skill) => (
                                <div key={skill.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                                  <div className="flex-1">
                                    <h5 className="text-foreground text-body-md font-medium">{skill.skill_name}</h5>
                                    <p className="text-muted-foreground text-caption-sm mt-1">
                                      Оценка от {new Date(skill.assessment_date).toLocaleDateString()}
                                    </p>
                                  </div>
                                  <div className="flex flex-col items-end">
                                    <div className="text-warning text-body-lg font-bold">{skill.skill_average?.toFixed(1)}</div>
                                    <div className="text-muted-foreground text-caption-sm">из 5</div>
                                  </div>
                                </div>
                              ))}
                            </div>
                            ...
                            <div className="space-y-3">
                              {skills.slice(0, 5).map((skill) => (
                                 <div key={skill.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                                  <div className="flex-1">
                                    <h5 className="text-foreground text-body-md font-medium">{skill.hard_skills.name}</h5>
                                    {skill.hard_skills.description && (
                                      <p className="text-muted-foreground text-caption-sm mt-1">{skill.hard_skills.description}</p>
                                    )}
                                  </div>
                                  <div className="flex flex-col items-end">
                                    <div className="text-warning text-body-lg font-bold">{skill.current_level}</div>
                                    <div className="text-muted-foreground text-caption-sm">из 5</div>
                                  </div>
                                </div>
                              ))}
                            ...
                            <div className="flex items-center justify-center py-4">
                              <p className="text-muted-foreground text-body-md">Нет данных о Hard Skills</p>
                            </div>
                          )}
                        </div>
                        ...
                        {/* Качества */}
                        <div>
                          <h4 className="text-foreground text-body-base font-semibold mb-4">Качества</h4>
                          {assessmentResultsLoading ? (
                            <div className="flex items-center justify-center py-4">
                              <p className="text-muted-foreground text-caption-sm">Загрузка...</p>
                            </div>
                            ...
                            <div className="space-y-3">
                              {qualityResults.slice(0, 5).map((quality) => (
                                <div key={quality.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                                  <div className="flex-1">
                                    <h5 className="text-foreground text-body-md font-medium">{quality.quality_name}</h5>
                                    <p className="text-muted-foreground text-caption-sm mt-1">
                                      Оценка 360° от {new Date(quality.assessment_date).toLocaleDateString()}
                                    </p>
                                  </div>
                                  <div className="flex flex-col items-end">
                                    <div className="text-warning text-body-lg font-bold">{quality.quality_average?.toFixed(1)}</div>
                                    <div className="text-muted-foreground text-caption-sm">из 5</div>
                                  </div>
                                </div>
                              ))}
                            ...
                            <div className="space-y-3">
                              {qualities.slice(0, 5).map((quality) => (
                                 <div key={quality.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                                  <div className="flex-1">
                                    <h5 className="text-foreground text-body-md font-medium">{quality.soft_skills.name}</h5>
                                    {quality.soft_skills.description && (
                                      <p className="text-muted-foreground text-caption-sm mt-1">{quality.soft_skills.description}</p>
                                    )}
                                  </div>
                                  <div className="flex flex-col items-end">
                                    <div className="text-warning text-body-lg font-bold">{quality.current_level}</div>
                                    <div className="text-muted-foreground text-caption-sm">из 5</div>
                                  </div>
                                </div>
                              ))}
                            ...
                            <div className="flex items-center justify-center py-4">
                              <p className="text-muted-foreground text-body-md">Нет данных о Soft Skills</p>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <Collapsible open={expandedSections.achievements} onOpenChange={(open) => setExpandedSections(prev => ({ ...prev, achievements: open }))}>
                <div className="w-full mt-2.5 pb-3 px-3 max-md:max-w-full">
                  <CollapsibleTrigger className="flex w-full items-center gap-[40px_100px] whitespace-nowrap justify-between flex-wrap max-md:max-w-full hover:opacity-80 transition-opacity">
                    ...
                    <div className="self-stretch flex items-center text-caption-sm text-warning font-normal text-right leading-none my-auto">
                      <span className="text-warning self-stretch w-[68px] my-auto">
                        {expandedSections.achievements ? 'Свернуть' : 'Развернуть'}
                      </span>
                      <ChevronDown className={`w-6 h-6 text-warning transition-transform ${expandedSections.achievements ? 'rotate-180' : ''}`} />
                    </div>
                  </CollapsibleTrigger>
                  
                  <CollapsibleContent className="w-full">
                    <div className="flex items-center justify-center py-8 w-full">
                      <p className="text-muted-foreground text-body-md">Нет наград</p>
                    </div>
                  </CollapsibleContent>
                </div>
              </Collapsible>
                ...
                <div className="w-full mt-2.5 pb-3 px-3 max-md:max-w-full">
                  <CollapsibleTrigger className="flex w-full items-center gap-[40px_100px] justify-between flex-wrap max-md:max-w-full hover:opacity-80 transition-opacity">
                    ...
                    <div className="self-stretch flex items-center text-caption-sm text-warning font-normal whitespace-nowrap leading-none my-auto">
                      ...
                      <ChevronDown className={`w-6 h-6 text-warning transition-transform ${expandedSections.career ? 'rotate-180' : ''}`} />
                    </div>
                  </CollapsibleTrigger>
                  ...
                  <CollapsibleContent className="w-full">
                    <div className="space-y-6 mx-3">
                      ...
                      <div className="space-y-4">
                        <div className="flex w-full gap-6 overflow-hidden p-3 rounded-[20px] bg-gradient-to-br from-chart-3 to-chart-3 max-md:max-w-full">
                          <div className="flex min-w-60 w-full flex-col items-stretch justify-center flex-1 shrink basis-[0%] pr-3 py-3">
                            <div className="flex w-full flex-col items-stretch justify-center max-md:max-w-full">
                              <h4 className="text-white text-ellipsis text-heading-4 font-semibold leading-none tracking-[-0.4px] max-md:max-w-full">
                                Пройдите Оценку 360
                              </h4>
                              <p className="text-muted text-caption-sm font-normal leading-none opacity-80 mt-2.5 max-md:max-w-full">
                            ...
                            <button 
                              ...
                              className={`justify-center items-center shadow-md backdrop-blur-[50px] flex gap-2.5 overflow-hidden text-body-md font-semibold leading-none mt-[22px] px-6 py-3 rounded-[32px] border-[1.5px] border-solid border-chart-3 transition-colors max-md:px-5 ${
                                canStart360Survey() 
                                  ? 'text-white hover:bg-background/10 cursor-pointer' 
                        ...
                        <div className="flex w-full gap-6 overflow-hidden p-3 rounded-[20px] bg-gradient-to-br from-accent to-accent max-md:max-w-full">
                          <div className="flex min-w-60 w-full flex-col items-stretch justify-center flex-1 shrink basis-[0%] pr-3 py-3">
                            <div className="flex w-full flex-col items-stretch justify-center max-md:max-w-full">
                              <h4 className="text-white text-ellipsis text-heading-4 font-semibold leading-none tracking-[-0.4px] max-md:max-w-full">
                            ...
                            <button 
                              ...
                              className={`justify-center items-center shadow-md backdrop-blur-[50px] flex gap-2.5 overflow-hidden text-body-md font-semibold leading-none mt-[22px] px-6 py-3 rounded-[32px] border-[1.5px] border-solid border-accent transition-colors max-md:px-5 ${
                                canStartSkillSurvey() 
                                  ? 'text-white hover:bg-background/10 cursor-pointer' 
                                  : 'text-white/50 cursor-not-allowed opacity-50'
                              }`}
                      ...
                      <div className="shadow-card flex w-full gap-6 flex-wrap bg-white p-6 rounded-[20px] max-md:max-w-full max-md:px-5">
                        <div className="w-full space-y-4">
                          <div className="text-caption-sm text-foreground font-normal leading-[17px]">
            ...
            <div className="w-full mt-2.5 pb-3 px-3 max-md:max-w-full">
              <CollapsibleTrigger className="flex w-full items-center gap-[40px_100px] justify-between flex-wrap max-md:max-w-full hover:opacity-80 transition-opacity">
                ...
                <div className="self-stretch flex items-center text-caption-sm text-warning font-normal whitespace-nowrap leading-none my-auto">
                  ...
                  <ChevronDown className={`w-6 h-6 text-warning transition-transform ${expandedSections.learning ? 'rotate-180' : ''}`} />
                </div>
              </CollapsibleTrigger>
            </div>
          ...
          {/* Services Section */}
          <section className="w-full text-foreground font-medium whitespace-nowrap mt-3 max-md:max-w-full">
            <div className="flex w-full items-center gap-2.5 text-body-lg tracking-[-0.18px] leading-none justify-center pt-5 pb-2.5 px-3 max-md:max-w-full">
            ...
            <div className="w-full text-body-md leading-none max-md:max-w-full">
              <button className="items-center border bg-foreground/5 flex w-full gap-5 overflow-hidden pl-3 pr-4 py-3 rounded-[10px] border-solid border-background/40 hover:bg-foreground/10 transition-colors max-md:max-w-full">
                <div className="self-stretch flex min-w-60 w-full items-center gap-[40px_100px] justify-between flex-wrap flex-1 shrink basis-[0%] my-auto max-md:max-w-full">
                  <div className="self-stretch flex items-center gap-4 my-auto">
                    <img
                      ...
                      alt="Support"
                    />
                    <span className="text-foreground opacity-80 self-stretch my-auto">
                      Поддержка
                    </span>
                  </div>
                  <img
