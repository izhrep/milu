# АКТУАЛЬНАЯ ДОКУМЕНТАЦИЯ ПРОЕКТА V8
...
13. [UI/UX особенности V8](#13-uiux-особенности-v8)
14. [RLS и безопасность](#14-rls-и-безопасность)
15. [UML-диаграммы](#15-uml-диаграммы)
...
- `/tasks` - мои задачи
- `/questionnaires` - опросники (обратная связь 360) ← **упрощен с /development/questionnaires**
...
- `/diagnostic-monitoring` - мониторинг диагностики ← **упрощен с /hr/diagnostic-monitoring**
...
### Обязательное согласие на cookies (V8)
...
CREATE TABLE users (
  ...
  cookies_consent BOOLEAN DEFAULT false,   -- V8
  cookies_consent_at TIMESTAMPTZ           -- V8
);
```
...
CREATE TABLE hard_skill_questions (
  ...
  order_index INTEGER,
  visibility_restriction_enabled BOOLEAN DEFAULT false,  -- V8
  visibility_restriction_type TEXT                       -- V8: 'self', 'manager', 'peer'
);
```
...
**Логика visibility_restriction (V8):**
...
CREATE TABLE hard_skill_answer_options (
  id UUID PRIMARY KEY,
  answer_category_id UUID REFERENCES answer_categories(id),
  title TEXT NOT NULL,            -- основной текст (V8: унифицировано)
  description TEXT,
  numeric_value INTEGER,
  level_value INTEGER,            -- 0-4 для Hard Skills (V8)
  order_index INTEGER
);
```
...
CREATE TABLE soft_skill_answer_options (
  ...
  description TEXT,
  numeric_value INTEGER,
  level_value INTEGER,            -- 0-5 для Soft Skills (V8)
  order_index INTEGER
);
```
...
CREATE TABLE hard_skill_results (
  ...
  is_skip BOOLEAN DEFAULT false,  -- V8: пропуск вопроса
  comment TEXT,
...
- При пропуске: `answer_option_id = NULL`, `numeric_value = NULL`, `level_value = NULL`
...
### Терминология V8 (обновлено)

| Старый термин | Новый термин V8 |
|---------------|-----------------|
| Оценка | Фидбек |
| Оценивающие | Респонденты |
| Оценка 360° | Обратная связь 360 |
| Самооценка | Личный фидбек |
| Оценка руководителя | Фидбек unit-лида |
| Оценка коллег | Обратная связь от коллег |
...
**RLS политика (V8):** `users_can_view_for_peer_selection`
```sql
...
### Ограничение видимости вопросов (V8)

Вопросы могут быть скрыты от определенных типов респондентов:
- `visibility_restriction_type = 'peer'` → скрыт от коллег
- `visibility_restriction_type = 'self'` → скрыт при самооценке
- `visibility_restriction_type = 'manager'` → скрыт от руководителя

---

## 6. Шкалы оценки компетенций

### Hard Skills: 0-4 (V8, обновлено с 0-3)
...
### Soft Skills: 0-5 (V8, обновлено с 0-4)
...
#### Фильтр по роли респондента (V8)
- Все
- Личный фидбек (self)
- Фидбек unit-лида (manager)
- Коллеги (peer)

#### Фильтр по категории должности (V8)
- Все
- [Динамически из position_categories]

### Динамические заголовки (V8)
...
Личный фидбек (1)
Фидбек unit-лида (1)
Средний фидбек коллег (N)
Все, кроме меня (N+1)
Все (N+2)
```

#### Детализация результатов (V8)
...
### Контекстные кнопки действий (V8)

| Тип задачи | Текст кнопки |
|------------|--------------|
| self assessment (новая) | "Пройти личный фидбек" |
| self assessment (draft) | "Продолжить личный фидбек" |
| peer evaluation (новая) | "Пройти фидбек коллеги" |
...
- Шифрование PII через Edge Function
- Хеширование пароля
...
| Функция | Назначение |
|---------|------------|
| `create-user` | Создание пользователя с шифрованием PII |
...
| `fix-double-encrypted-user` | Исправление двойного шифрования |
...
FOR INSERT WITH CHECK (
  -- Руководитель добавляет для своего подчиненного
  EXISTS (
...
**Шифруемые поля:** email, first_name, last_name, middle_name

**Процесс:**
1. Фронтенд отправляет plain text в Edge Function
2. Edge Function вызывает Yandex Cloud Function для шифрования
3. Зашифрованные данные сохраняются в БД
4. При чтении - расшифровка через Yandex Cloud Function на фронтенде
...
sequenceDiagram
    ...
    Note over Employee,System: Может пропустить вопросы
...
1. **Шкалы оценки:**
   - Hard Skills: 0-4 (было 0-3)
   - Soft Skills: 0-5 (было 0-4)
...
3. **Маршрутизация:**
   ...
   - `/hr/diagnostic-monitoring` → `/diagnostic-monitoring`

4. **UI/UX:**
   - Плоская структура сайдбара без группирующих заголовков
   - Пункт "Профиль" скрыт для employee
   - Условная видимость "Карьерный трек" и "Встречи 1:1" для employee
   - Пропуск вопросов "Не могу ответить"
   - Ограничение видимости вопросов по типу респондента
...
6. **Форма выбора респондентов:**
   - Фильтры по должности и категории должности
   - Доступ ко всем пользователям компании (employee/manager)

7. **Безопасность:**
   - Обязательное согласие на cookies при входе
   - RLS политика для выбора респондентов

8. **Унификация:**
   - Поле `title` вместо `label` в answer_options
   - Единый формат данных для импорта
